...
const BonusBetCalculator = ({ onReceiveBonusBet }) => {
  ...
  const handleCalculate = () => (e) => {
    if (isInputsValid([bonusBet, oddsOne, oddsTwo])) {
      const bonusBetCalculation = calculateBonusBet(
        outcome1,
        outcome2,
        oddsOne,
        oddsTwo,
  ...
  const handleAddBonusBet = () => {
    if (isInputsValid([bonusBet, oddsOne, oddsTwo])) {
      const { stake, profit, bonus, other } = calculateBonusBet(
        outcome1,
        outcome2,
        oddsOne,
        oddsTwo,
    ...
    return (
      <>
        <Grid item lg={4} md={6} xs={12}>
          <Typography
            variant="h6"
            className={classes.resultTextBonus}
          >{`Use the £${calculation.bonusBet} bonus on ${calculation.bonus}`}</Typography>
        </Grid>
        <Grid item lg={4} md={6} xs={12}>
          <Typography
            variant="h6"
            className={classes.resultTextBonus}
          >{`Stake £${calculation.stake} on ${calculation.other}`}</Typography>
        </Grid>
        <Grid item lg={4} md={6} xs={12}>
          <Typography
            variant="h6"
            className={classes.resultTextBonus}
          >{`Total profit for either outcome £${calculation.profit}`}</Typography>
        </Grid>
      </>
    );
  };
    ...
    <Grid container className={classes.container}>
      <Grid item lg={2} md={6} xs={12}>
        <TextField
          ...
          value={bonusBet}
          onChange={(e) =>
            isValidInput(e.target.value) && setBonusBet(e.target.value)
          }
      ...
      <Grid item lg={2} md={6} xs={12}>
        <TextField
          ...
          value={oddsOne}
          onChange={(e) =>
            isValidInput(e.target.value) && setOddsOne(e.target.value)
          }
          className={classes.selection}
        />
      </Grid>
      ...
      <Grid item lg={2} md={6} xs={12}>
        <TextField
          ...
          value={oddsTwo}
          onChange={(e) =>
            isValidInput(e.target.value) && setOddsTwo(e.target.value)
          }
          className={classes.selection}
        />
      </Grid>
